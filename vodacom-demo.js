(() => {
  ...
  const icons = { data: '⌁', airtime: '◉', recharge: '＋', bundles: '◫', send: '↗', bill: '▤', bell: '♧', close: '×', home: '⌂', buy: '▱', rewards: '★', more: '⋮' };
  const toast = (text) => {
  ...
  const open = () => {
    ...
    const openFlow = (key) => {
      ...
      app.querySelector('#flowSummary').innerHTML = `<div><span>Payment source</span><b>Yami main balance</b></div><div><span>Available</span><b>R${yamiBalance.toLocaleString('en-ZA', { minimumFractionDigits: 2 })}</b></div>`;
      app.querySelector('#flowConfirm').textContent = key === 'balances' ? 'Close details' : 'Continue';
      app.querySelector('#flowConfirm').dataset.stage = 'select';
    ...
    app.querySelector('#flowConfirm').onclick = () => {
      ...
      const confirm = app.querySelector('#flowConfirm');
      if (confirm.dataset.stage === 'select') {
        app.querySelector('#flowSummary').innerHTML = `<div><span>${activeFlow.title}</span><b>${label}</b></div>${activeFlow.input ? `<div><span>${activeFlow.input}</span><b>${input.value}</b></div>` : ''}<div><span>Charge from Yami main balance</span><b>R${amount.toFixed(2)}</b></div><div><span>Remaining balance</span><b>R${(yamiBalance - amount).toLocaleString('en-ZA', { minimumFractionDigits: 2 })}</b></div>`;
        confirm.textContent = amount ? `Confirm and pay R${amount.toFixed(2)}` : 'Close details'; confirm.dataset.stage = 'confirm';
        return;
      }
    ...
    app.querySelectorAll('.telecom-nav:not(.active)').forEach(el => el.onclick = () => toast(`${selected.name}: ${el.textContent.trim().replace(/\s+/g, ' ')} selected`));
  };
