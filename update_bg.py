...
for file in files:
    ...
    if 'bg_splash.png' in content:
        # Regex to replace the background
        content = re.sub(r"background:[^;]+url\('assets/images/main/bg_splash\.png'\)[^;]*;", "background:linear-gradient(rgba(0,0,0,0.4), rgba(0,0,0,0.95)), url('https://images.unsplash.com/photo-1534447677768-be436bb09401?auto=format&fit=crop&q=80&w=1000') center/cover fixed !important;", content)
        
        if 'yami-watermarks' not in content:
